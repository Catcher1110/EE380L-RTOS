...
void Task2(void){
  ...
  for(;;){
    Count2++;
		GPIO_PORTF_DATA_R = 0x00;        // toggle the LED
...
int Testmain(void){
  ...
  OS_AddThread(&Task1, 128, 3);
	OS_AddThread(&Task2, 128, 3);
	OS_AddThread(&Task3, 128, 3);
  OS_Launch(1000*TIMESLICE); // doesn't return, interrupts enabled in here
