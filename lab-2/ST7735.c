...
// **************** Extend Version LCD ***************
// Two logically sepreate displays
// Inputs:  device is the device on which you want to display (specify top 0 or bottom 1)
//				  line is the line on which you want to display (specify the line number 0-3)
//          string is a pointer null terminated ACSII string
//				  value is the number to display
// Outputs: none
void ST7735_Message(int device, int line, char *string, uint32_t value){
	OS_Wait(&LCDFree); // capture resource
	int offset;
	int NumOfString;
	int liney;
	ST7735_DrawFastHLine(0,79,128,ST7735_Color565(0,255,0));
	// Draw the line that split the LCD into two parts
	if(device == 0){
		offset = 0;
	}else if(device == 1){
		offset = 8;
	}
	liney = line*2 + offset + 1;
	NumOfString = ST7735_DrawString(0, liney, string, ST7735_Color565(0, 255, 0));
	StX=NumOfString+2;
	StY=liney;
	ST7735_OutUDec(value);
	//ST7735_DrawNumber(NumOfString*6, liney, value, ST7735_Color565(0, 255, 0));
	OS_Signal(&LCDFree); // release resource
}
