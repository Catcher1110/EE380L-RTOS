...
#define PF3  (*((volatile unsigned long *)0x40025020))
  ...
  GPIO_PORTF_DIR_R |= 0x0F;                // make PF2 out (built-in LED)
  GPIO_PORTF_AFSEL_R &= ~0x0F;             // disable alt funct on PF2
  GPIO_PORTF_DEN_R |= 0x0F;                // enable digital I/O on PF2
...
void SW1Push(void){
  if(OS_SWMsTime() > 20){ // debounce
    if(OS_AddThread(&ButtonWork,128,2)){
      NumCreated++; 
    }
    OS_SWClearMsTime();  // at least 20ms between touches
  }
}
//************SW2Push*************
...
//unsigned long Count;   // number of times thread5 loops
...
long TaskAJitter;
long TaskBJitter;
unsigned long TaskAJitterHistogram[JITTERSIZE]={0,};
unsigned long TaskBJitterHistogram[JITTERSIZE]={0,};
#define periodA TIME_1MS
#define periodB 2*TIME_1MS

extern void Jitter(void);   // prints jitter information (write this)
void Jitter(void)
{
		UART_OutString("Task A and Task B Jitter: ");
		UART_OutUDec(TaskAJitter);
		UART_OutChar(' ');
		UART_OutUDec(TaskBJitter);
}
...
void TaskA(void){       // called every {1000, 2990us} in background
	unsigned static long ALastTime;  // time at previous ADC sample
	unsigned long AthisTime;         // time at current ADC sample
	long Ajitter;                    // time between measured and expected, in us
	static uint8_t Aignore = 0;
	
  PE1 = 0x02;      // debugging profile  
  CountA++;
  PseudoWork(workA*counts1us); //  do work (100ns time resolution)

	AthisTime = OS_Time();       // current time, 12.5 ns
//  if(Aignore%1000 == 0){
//	  UART_OutUDec(AthisTime);
//		OutCRLF();
//	}
	Aignore++;        // calculation finished
	if(Aignore>1){    // ignore timing of first interrupt
//		if(ALastTime < AthisTime){
//		   UART_OutUDec(111);
//	     OutCRLF();
//		}

		unsigned long Adiff = OS_TimeDifference(ALastTime,AthisTime);
		if(Adiff>periodA){
			Ajitter = (Adiff-periodA+4)/8;  // in 0.1 usec
		}else{
			Ajitter = (periodA-Adiff+4)/8;  // in 0.1 usec
		}
		if(Ajitter > TaskAJitter){
			TaskAJitter = Ajitter; // in usec
		}       // jitter should Ae 0
		if(Ajitter >= JitterSize){
			Ajitter = JITTERSIZE-1;
		}
		TaskAJitterHistogram[Ajitter]++; 
	}
	ALastTime = AthisTime;
	//UART_OutString("TaskA");
	
  PE1 = 0x00;      // debugging profile  
}
#define workB 250       // 250 us work in Task B
void TaskB(void){       // called every pB in background
	unsigned static long BLastTime;  // time at previous ADC sample
	unsigned long BthisTime;         // time at current ADC sample
	long Bjitter;                    // time between measured and expected, in us
	static uint8_t Bignore = 0;
	
  PE2 = 0x04;      // debugging profile  
  CountB++;
  PseudoWork(workB*counts1us); //  do work (100ns time resolution)
	
	BthisTime = OS_Time();       // current time, 12.5 ns
//  if(Bignore%1000 == 0){
//	  UART_OutUDec(BthisTime);
//		OutCRLF();
//	}
	Bignore++;        // calculation finished
	if(Bignore>1){    // ignore timing of first interrupt
		unsigned long Bdiff = OS_TimeDifference(BLastTime,BthisTime);
		if(Bdiff>periodB){
			Bjitter = (Bdiff-periodB+4)/8;  // in 0.1 usec
		}else{
			Bjitter = (periodB-Bdiff+4)/8;  // in 0.1 usec
		}
		if(Bjitter > TaskBJitter){
			TaskBJitter = Bjitter; // in usec
		}       // jitter should be 0
		if(Bjitter >= JitterSize){
			Bjitter = JITTERSIZE-1;
		}
		TaskBJitterHistogram[Bjitter]++; 
	}
	BLastTime = BthisTime;
//	UART_OutString("TaskB");
//	OutCRLF();	
...
int Testmain5(void){       // Testmain5 Lab 3
  ...
  OS_AddPeriodicThread(&TaskA,periodA,0);           // 1 ms, higher priority
  OS_AddPeriodicThread(&TaskB,periodB,1);         // 2 ms, lower priority
