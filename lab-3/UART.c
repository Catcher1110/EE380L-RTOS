...
// Modified by EE380L students Huang Huang && Jian Chu
...
void UART_Init(void){
  ...
  UART0_IBRD_R = 43;                    // IBRD = int(80,000,000 / (16 * 115,200)) = int(27.1267)
...
//------------UART_InNum------------
// Accepts ASCII Numbers from the serial port
//    and adds them to a string until <enter> is typed
//    or until max length of the string is reached.
// It echoes each character as it is inputted.
// If a backspace is inputted, the string is modified
...
// Input: pointer to empty buffer, size of buffer
// Output: Null terminated string
// -- Modified by Huang Huang && Jian Chu --
void UART_InNum(char *bufPt, uint16_t max) {
int length=0;
char character;
  character = UART_InChar();
  while(character != CR){ // accepts until <enter> is typed
...
// changed and a backspace is outputted to the screen
    if(character==BS){
		...
		else if((character>='0') && (character<='9')&&(length < max)) {
			*bufPt = character;
      bufPt++;
      length++;
      UART_OutChar(character);
