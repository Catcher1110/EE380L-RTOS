...
// Provide a function that initializes Timer to trigger ADC
...
#define SYSCTL_RCGCGPIO_R1      0x00000002  // GPIO Port B Run Mode Clock
                                            // Gating Control
extern uint32_t ADC_collect_status;
...
void ADC_Init(uint8_t channelNum){
  ...
  TIMER2_TAMR_R = 0x00000002;   // configure for periodic mode, default down-count settings
  TIMER2_TAPR_R = 0;            // prescale value for trigger
  TIMER2_TAILR_R = 200000-1;    // start value for trigger
  TIMER2_IMR_R = 0x00000001;    // disable all interrupts
  TIMER2_CTL_R |= 0x00000001;   // enable timer0A 32-b, periodic, no interrupts
  ADC0_PC_R = 0x01;         // configure for 125K samples/sec
  ADC0_SSPRI_R = 0x3210;    // sequencer 0 is highest, sequencer 3 is lowest
...
// Outputs: None
volatile uint32_t ADCvalue;
void (*ADCTask)(uint32_t data);
// ***************** ADC0Seq3_Handler ****************
// Handler for interrupt, read the data from the ADC
// Inputs:  None
// Outputs: None
...
void ADC_Collect(uint32_t channelNum, uint32_t fs, void(*task)(uint32_t data)){
