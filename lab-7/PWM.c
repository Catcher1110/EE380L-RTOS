...
#define DISTANCE_THRESHOLD1 87
#define DISTANCE_THRESHOLD2 57
#define DISTANCE_THRESHOLD3 37
#define ERROR 87
...
uint32_t RThres = 0;
uint32_t LThres = 0;
uint32_t RDistance = 0 ;
uint32_t LDistance = 0;
uint32_t FDistance = 0;

void SetRDistance (uint32_t dis){
	RDistance = dis;
}
void SetLDistance (uint32_t dis){
	LDistance = dis;
}
void SetFDistance (uint32_t dis){
	FDistance = dis;
}
void SetRThres (uint32_t dis){
	RThres = dis;
}
void SetLThres (uint32_t dis){
	LThres = dis;
}


/*void DistanceToPowerConverter(){
	PowerRef = FDistance * DISTANCE_TO_POWER_CONVERSION_FACTOR;
}*/
/*void DistanceToPowerDifeerenceConverter(){
	int distance = RDistance - LDistance;
	PowerDiff = distance * DISTANCE_TO_POWER_DIFFERENCE_CONVERSION_FACTOR;
}*/
/*void DistanceToDirectionConverter(){
	int distance = RDistance - LDistance;
	if(distance > 0){
...
/*void Interface(){
	while(1){
	  DistanceToPowerConverter();
	  // DistanceToPowerDifeerenceConverter();
	  // DistanceToDirectionConverter();
		if(RDistance>LDistance + ERROR){
			if(RThres>DISTANCE_THRESHOLD1){
		    Servo_Deg_Duty(89);
	      DCL_Motor(Direction, Direction,POWERMAX);
	      DCR_Motor(Direction, Direction, POWERMIN);
	    }
			else if(RThres>DISTANCE_THRESHOLD2){
		    Servo_Deg_Duty(59);
	      DCL_Motor(Direction, Direction,POWERMAX);
	      DCR_Motor(Direction, Direction, PowerRef);
	    }
			else if(RThres>DISTANCE_THRESHOLD3){
		    Servo_Deg_Duty(29);
	      DCL_Motor(Direction, Direction,POWERMAX);
	      DCR_Motor(Direction, Direction, PowerRef);
	    }
		    ...
		    Servo_Deg_Duty(0);
	      DCL_Motor(Direction, Direction,POWERMIN);
	      DCR_Motor(Direction, Direction, POWERMIN);
	    }
		}
		else if (LDistance > RDistance + ERROR){
			if(LThres>DISTANCE_THRESHOLD1){
		    Servo_Deg_Duty(89);
	      DCL_Motor(Direction, Direction, POWERMIN);
	      DCR_Motor(Direction, Direction, POWERMAX);
	    }
			else if(LThres>DISTANCE_THRESHOLD2){
		    Servo_Deg_Duty(59);
	      DCL_Motor(Direction, Direction,PowerRef);
	      DCR_Motor(Direction, Direction, POWERMAX);
	    }
			else if(LThres>DISTANCE_THRESHOLD3){
		    Servo_Deg_Duty(29);
	      DCL_Motor(Direction, Direction,PowerRef);
	      DCR_Motor(Direction, Direction, POWERMAX);
	    }
...
//	PLL_Init(Bus80MHz);  // bus clock at 80 MHz
  ...
  DRV8848_LeftInit(PERIOD10MS, POWERMIN, FORWARD);
  DRV8848_RightInit(PERIOD10MS, POWERMIN, FORWARD);
}


void Servo_Deg_Duty(int direction, int deg){
	if(deg > 90){
	deg = 90;
	}
if (direction == 0){
  ...
  /*if (direction == 0){	
    duty = deg/90 + 1.5; //convert degree to ms
    duty = duty * 25000/20;  // convert ms to peiod units
  }
	else {
		duty = 1.5 - deg/90; //convert degree to ms
    duty = duty * 25000/20;  // convert ms to peiod units
	}*/
...
void DCL_Motor(int direction, int* LastDirectionL,uint32_t power){
          ...
          //direction = 1;         
			...
			if(power > POWERMAX){
        power = POWERMAX;      // go back to minimum
      }
      //PF2 ^= 0x04;
      /*if(power == POWERMIN){
        DRV8848_LeftStop();}*/
...
void DCR_Motor(int direction,int* LastDirectionR, uint32_t power){
